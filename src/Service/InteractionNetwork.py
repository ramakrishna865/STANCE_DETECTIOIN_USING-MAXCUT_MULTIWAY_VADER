...
class InteractionNetwork:
    ...
    def buildInteractionNetwork(self):
        ...
        self.graph.add_node(str(self.conversationTree.initiatedAuthorId))
        for node in self.conversationTree.tree.keys():
            u = self.conversationTree.initiatedAuthorId
            if node != self.conversationTree.initiatedAuthorId:
